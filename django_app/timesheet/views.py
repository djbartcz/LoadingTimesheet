...
def get_excel_timezone():
    ...
    if excel_tz.lower() == 'system':
        ...
        if isinstance(local_tz, pytz.BaseTzInfo):
            ...
            mapped_tz = tz_mapping.get(tz_name)
            if mapped_tz:
...
def timer_page(request, employee_id):
    ...
    try:
        ...
        non_productive_tasks = []
        seen_prod_names = set()
        seen_non_prod_names = set()
        for code, item in mapping.items():
            report_cost_code = str(code or '').strip()
            if not report_cost_code:
                continue
            if not bool(item.get('is_enabled', True)):
                continue
            display_name = str(item.get('activity_name') or '').strip()
            if not display_name:
                continue
            is_non_productive = bool(item.get('is_non_productive', False))
            if is_non_productive:
                if display_name in seen_non_prod_names:
                    continue
                seen_non_prod_names.add(display_name)
                non_productive_tasks.append({"name": display_name})
            else:
                if display_name in seen_prod_names:
                    continue
                seen_prod_names.add(display_name)
                tasks.append(
                    {
                        "name": display_name,
        ...
        mapping_entries.append(
            {
                ...
                'is_enabled': bool(map_item.get('is_enabled', True)),
            }
        )
...
def refresh_master_data_from_ifs(request):
    ...
    try:
        ...
        for item in activities_payload.get('value', []):
            ...
            if map_item and not bool(map_item.get('is_enabled', True)):
                continue
            task_name = str(
...
def edit_time_records(request):
    ...
    for code, item in mapping.items():
        ...
        if is_non_productive:
            if display_name in seen_non_prod_names:
                continue
            seen_non_prod_names.add(display_name)
            non_productive_tasks.append(
                type('Task', (), {'name': display_name})()
            )
        else:
            if display_name in seen_prod_names:
                continue
            seen_prod_names.add(display_name)
            productive_tasks_data.append(
                {'name': display_name, 'report_cost_code': report_cost_code}
            )
    productive_tasks_data.sort(key=lambda x: x['name'].lower())
    non_productive_tasks.sort(key=lambda x: x.name.lower())
    productive_task_names = {t['name'] for t in productive_tasks_data}
    non_productive_task_names = {t.name for t in non_productive_tasks}
    ...
    return render(request, 'timesheet/edit_time_records.html', {
        ...
        'productive_task_names': productive_task_names,
        'non_productive_task_names': non_productive_task_names,
        'filters': {
...
def send_records_to_ifs(request):
    ...
    def resolve_report_cost_code(task_name):
        """Return (code, error) tuple. code is None on failure."""
        normalized = str(task_name or '').strip().lower()
        if not normalized:
            return None, 'Empty task name'
        disabled_match = None
        for code, item in mapping.items():
            ...
            report_cost_name = (
                str(item.get('report_cost_name') or '').strip().lower()
            )
        ...
        if disabled_match:
            return None, (
                f'Task "{task_name}" maps to code '
                f'"{disabled_match}" which is disabled in admin'
            )
        return None, (
            f'Could not map task "{task_name}" to any '
            f'ReportCostCode'
        )

    ALLOWED_CODES_54 = {
        code for code, item in mapping.items()
        if not bool(item.get('is_non_productive', False))
        and bool(item.get('is_enabled', True))
    }
    ALLOWED_CODES_NON_PROD = {
        code for code, item in mapping.items()
        if bool(item.get('is_non_productive', False))
        and bool(item.get('is_enabled', True))
    }
    ALLOWED_CODES_MC = ALLOWED_CODES_54 | ALLOWED_CODES_NON_PROD
    ...
    for record_id in record_ids:
        ...
        if record.is_non_productive:
            ...
            report_cost_code, resolve_err = resolve_report_cost_code(
                record.task
            )
            if not report_cost_code:
                if 'disabled' in (resolve_err or ''):
                    results.append(
                        {
                            'id': str(record.id),
                            'success': True,
            ...
            if project_company and project_company != "54":
                activity_report_code, resolve_err = (
                    resolve_report_cost_code(record.task)
                )
